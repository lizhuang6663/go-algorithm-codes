...
接着递归调用 dfs(node.Left, left, x)，即 dfs(节点 3, math.MinInt, 5) 处理左子树；同时递归调用 dfs(node.Right, x, right)，即 dfs(节点 7, 5, math.MaxInt) 处理右子树。

处理左子树节点 3
当前节点 node 为 3，left = math.MinInt，right = 5。
节点 3 的值 x = 3，满足 math.MinInt < 3 && 3 < 5。
接着递归调用 dfs(node.Left, left, x)，即 dfs(节点 2, math.MinInt, 3) 处理左子树；同时递归调用 dfs(node.Right, x, right)，即 dfs(节点 4, 3, 5) 处理右子树。

处理左子树节点 2
当前节点 node 为 2，left = math.MinInt，right = 3。
节点 2 的值 x = 2，满足 math.MinInt < 2 && 2 < 3。
由于节点 2 没有左子树和右子树，递归调用 dfs(node.Left, left, x) 和 dfs(node.Right, x, right) 时，传入的节点为 nil，根据 dfs 函数的逻辑，会返回 true。

处理左子树节点 4
当前节点 node 为 4，left = 3，right = 5。
节点 4 的值 x = 4，满足 3 < 4 && 4 < 5。
由于节点 4 没有左子树和右子树，递归调用 dfs(node.Left, left, x) 和 dfs(node.Right, x, right) 时，传入的节点为 nil，会返回 true。

处理右子树节点 7
当前节点 node 为 7，left = 5，right = math.MaxInt。
节点 7 的值 x = 7，满足 5 < 7 && 7 < math.MaxInt。
接着递归调用 dfs(node.Right, x, right)，即 dfs(节点 8, 7, math.MaxInt) 处理右子树。

处理右子树节点 8
当前节点 node 为 8，left = 7，right = math.MaxInt。
节点 8 的值 x = 8，满足 7 < 8 && 8 < math.MaxInt。
由于节点 8 没有左子树和右子树，递归调用 dfs(node.Left, left, x) 和 dfs(node.Right, x, right) 时，传入的节点为 nil，会返回 true。
...
进入 dfs 函数处理根节点 5
首先递归调用 dfs 处理左子树，即 dfs(节点 3)，以及右子树，即 dfs(节点 7)。

处理左子树节点 3
对于节点 3，同样要递归调用 dfs 处理其左子树 dfs(节点 2) 和右子树 dfs(节点 4)。

处理左子树节点 2
因为节点 2 没有左右子树，调用 dfs 时传入 nil，根据代码逻辑，dfs(nil) 会返回 (math.MaxInt, math.MinInt)。这里 math.MaxInt 作为初始的最小值，math.MinInt 作为初始的最大值，方便后续比较。
所以对于节点 2，lMin = math.MaxInt，lMax = math.MinInt。
节点 2 的值 x = 2，此时返回 (min(math.MaxInt, 2), max(math.MinInt, 2))，即 (2, 2)。

处理左子树节点 4
节点 4 也没有左右子树，dfs(nil) 返回 (math.MaxInt, math.MinInt)。
...
处理右子树节点 7
节点 7 有右子树 dfs(节点 8)，节点 8 没有左右子树，dfs(nil) 返回 (math.MaxInt, math.MinInt)。
节点 8 的值 x = 8，返回 (8, 8)。
对于节点 7，左子树为空，dfs(nil) 返回 (math.MaxInt, math.MinInt)，右子树（节点 8）结果为 rMin = 8，rMax = 8。
...
回到 isValidBST 函数
isValidBST 函数接收 dfs(root) 的返回值，_, mx := dfs(root) 得到 mx = 8。
